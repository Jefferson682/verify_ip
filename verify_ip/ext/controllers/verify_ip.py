    ...
    if ip_old:
        if ip_actual == ip_old[0][0]:
            ...
            try:
                ...
                insert_new_ip = connection_db.alterData(
                    f"INSERT INTO ip(ip_number, ip_active) VALUES('{ip_actual}', 1)"
                )
                inactive_old_ip = connection_db.alterData(
                    f"UPDATE ip SET ip_active = 0 WHERE ip_number = '{ip_old[0][0]}'"
                )
